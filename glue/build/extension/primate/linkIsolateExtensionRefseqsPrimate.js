...
// list the sequences in source ncbi-refseqs-siv
var listSeqResult = glue.command(["list", "sequence", "-w", "source.name = 'ncbi-refseqs-siv'"]);
...
_.each(seqIds, function(seqId) {
    ...
    // associate the corresponding sequence with this object.
    glue.inMode("sequence/ncbi-refseqs-siv/"+seqId, function() {
        glue.command(["set", "link-target", "isolate_data", "custom-table-row/isolate_data/"+seqId]);
    });

    glue.inMode("sequence/ncbi-refseqs-siv/"+seqId, function() {
        
        glue.command(["set", "field", "species", 'SIV']);
        glue.command(["set", "field", "subgenus", 'Similentivirus']);
